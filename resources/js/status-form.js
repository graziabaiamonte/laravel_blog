...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.status-form').forEach((form) => {
        form.addEventListener('submit', async (event) => {
            ...
            const nuovoStato = form.querySelector('select[name="status"]').value;
            ...
            try {
                ...
                const badge = document.querySelector('[data-status-badge="' + articleId + '"]');
                if (badge) {
                    badge.textContent = dati.label; 
                    badge.className = 'status-badge ' +
                        (dati.isPublished ? 'status-badge--published' : 'status-badge--draft');
                }
